...
String buildTestsDart(var config) {
    ...
    if (config['language'] == 'java') {
        ...
        tests = buildOther(config['tests'], config['language']);
...
String buildOther(var tests, var lang) {
    String output = "";
    for (var test in tests) {
        ...
        if (test.containsKey('args') && test.containsKey('input')) {
            io = 'new InputOutput.withArgsInput(${encode("name")}, ${encode("args")}, ${encode("input")}, ${encode("output")}, $points)';
        } else if (test.containsKey('args')) {
            io = 'new InputOutput.withArgs(${encode("name")}, ${encode("args")}, ${encode("output")}, $points)';
        } else if (test.containsKey('input')) {
            io = 'new InputOutput(${encode("name")}, ${encode("input")}, ${encode("output")}, $points)';
        } else {
            io = 'new InputOutput(${encode("name")}, "", ${encode("output")}, $points)';
        }
        String command = JSON.encode("$lang ${test['run']}");
        output += "targets.add(IOTarget.make($command, $io));";
    }
    return output;
}
...
String buildJava(var tests) {
    ...
    for (var comp in compiles.keys) {
        ...
        for (var test in compiles[comp]) {
            ...
            if (test.containsKey('args') && test.containsKey('input')) {
                ios += 'new InputOutput.withArgsInput(${encode("name")}, ${encode("args")}, ${encode("input")}, ${encode("output")}, $points)';
            } else if (test.containsKey('args')) {
                ios += 'new InputOutput.withArgs(${encode("name")}, ${encode("args")}, ${encode("output")}, $points)';
            } else if (test.containsKey('input')) {
                ios += 'new InputOutput(${encode("name")}, ${encode("input")}, ${encode("output")}, $points)';
            } else {
                ios += 'new InputOutput(${encode("name")}, "", ${encode("output")}, $points)';
            }
            ios += ', ';
        }
